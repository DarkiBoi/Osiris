...
public class Nametags extends Module {
    ...
    public void setup(){
        OsirisMod.getInstance().settingsManager.rSetting(scale = new Setting("ntagScale", this, 1, 0.1, 10, false));
    }
    ...
    private void drawNametag(Entity entityIn) {
        ...
        float m = (f / 8f) * (float) (Math.pow(1.2589254f, scale.getValDouble() / 10));
        GlStateManager.scale(m, m, m);

        FontRenderer fontRendererIn = mc.fontRenderer;
        GlStateManager.scale(-0.025F, -0.025F, 0.025F);

        String str = entityIn.getName() + " " + "\u00A7" + "a" + Math.round(((EntityLivingBase) entityIn).getHealth() + (entityIn instanceof EntityPlayer ? ((EntityPlayer) entityIn).getAbsorptionAmount() : 0));
        ...
        bufferbuilder.pos(-i - 1, 8, 0.0D).color(0.0F, 0.0F, 0.0F, 0.3F).endVertex();
        bufferbuilder.pos(-i - 1, 19, 0.0D).color(0.0F, 0.0F, 0.0F, 0.3F).endVertex();
        bufferbuilder.pos(i + 1, 19, 0.0D).color(0.0F, 0.0F, 0.0F, 0.3F).endVertex();
        bufferbuilder.pos(i + 1, 8, 0.0D).color(0.0F, 0.0F, 0.0F, 0.3F).endVertex();
        tessellator.draw();
         */
        ...
        bufferbuilder.pos(-i - 1, 8, 0.0D).color(.1f, .1f, .1f, .01f).endVertex();
        bufferbuilder.pos(-i - 1, 19, 0.0D).color(.1f, .1f, .1f, .01f).endVertex();
        bufferbuilder.pos(i + 1, 19, 0.0D).color(.1f, .1f, .1f, .01f).endVertex();
        bufferbuilder.pos(i + 1, 8, 0.0D).color(.1f, .1f, .1f, .01f).endVertex();
        tessellator.draw();
        ...
        int length = (array = enchants).length; for (int i = 0; i < length; i++) {
            ...
            if (level > 0) {
                ...
                fontRenderer.drawString("\u00a7f" + enchant.getName() + " " + levelDisplay,
                        20 - fontRenderer.getStringWidth("\u00a7f" + enchant.getName() + " " + levelDisplay) / 2f, 0, Color.WHITE.getRGB(), true);
                GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
                GlStateManager.enableLighting();
                GlStateManager.enableDepth();
    ...
    public void drawDamage(ItemStack itemstack,int x,int y) {
        ...
        Color c = new Color((int)(red * 255), (int)(green * 255), 0);
        GlStateManager.disableDepth();
        mc.fontRenderer.drawStringWithShadow(dmg + "", x + 8 - mc.fontRenderer.getStringWidth(dmg + "") / 2f, y-11, c.getRGB());
        GlStateManager.enableDepth();
    }
